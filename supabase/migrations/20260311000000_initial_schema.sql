...
create policy "Users can view own entries"
  on "public"."matcha_entries" as permissive for select to public
  using ((( SELECT auth.uid() AS uid) = user_id));

create policy "Users can insert own entries"
  on "public"."matcha_entries" as permissive for insert to public
  with check ((( SELECT auth.uid() AS uid) = user_id));

create policy "Users can update own entries"
  on "public"."matcha_entries" as permissive for update to public
  using ((( SELECT auth.uid() AS uid) = user_id));

create policy "Users can delete own entries"
  on "public"."matcha_entries" as permissive for delete to public
  using ((( SELECT auth.uid() AS uid) = user_id));

create policy "Users can view own taste analysis"
  on "public"."taste_analysis" as permissive for select to public
  using ((EXISTS ( SELECT 1 FROM public.matcha_entries e
    WHERE ((e.id = taste_analysis.entry_id) AND (e.user_id = ( SELECT auth.uid() AS uid))))));

create policy "Users can insert own taste analysis"
  on "public"."taste_analysis" as permissive for insert to public
  with check ((EXISTS ( SELECT 1 FROM public.matcha_entries e
    WHERE ((e.id = taste_analysis.entry_id) AND (e.user_id = ( SELECT auth.uid() AS uid))))));

create policy "Users can update own taste analysis"
  on "public"."taste_analysis" as permissive for update to public
  using ((EXISTS ( SELECT 1 FROM public.matcha_entries e
    WHERE ((e.id = taste_analysis.entry_id) AND (e.user_id = ( SELECT auth.uid() AS uid))))));

create policy "Users can delete own taste analysis"
  on "public"."taste_analysis" as permissive for delete to public
  using ((EXISTS ( SELECT 1 FROM public.matcha_entries e
    WHERE ((e.id = taste_analysis.entry_id) AND (e.user_id = ( SELECT auth.uid() AS uid))))));

create policy "Users can view own flavor profiles"
  on "public"."flavor_profiles" as permissive for select to public
  using ((EXISTS ( SELECT 1 FROM public.matcha_entries e
    WHERE ((e.id = flavor_profiles.entry_id) AND (e.user_id = ( SELECT auth.uid() AS uid))))));

create policy "Users can insert own flavor profiles"
  on "public"."flavor_profiles" as permissive for insert to public
  with check ((EXISTS ( SELECT 1 FROM public.matcha_entries e
    WHERE ((e.id = flavor_profiles.entry_id) AND (e.user_id = ( SELECT auth.uid() AS uid))))));

create policy "Users can update own flavor profiles"
  on "public"."flavor_profiles" as permissive for update to public
  using ((EXISTS ( SELECT 1 FROM public.matcha_entries e
    WHERE ((e.id = flavor_profiles.entry_id) AND (e.user_id = ( SELECT auth.uid() AS uid))))));

create policy "Users can delete own flavor profiles"
  on "public"."flavor_profiles" as permissive for delete to public
  using ((EXISTS ( SELECT 1 FROM public.matcha_entries e
    WHERE ((e.id = flavor_profiles.entry_id) AND (e.user_id = ( SELECT auth.uid() AS uid))))));
